...
import (
	...
	"k8s.io/node-problem-detector/pkg/netchecker"
)
...
func main() {
	...
	hco := options.NewNetcheckerOptions()
	...
	hc, err := netchecker.Newnetchecker(hco)
	if err != nil {
		fmt.Println(err)
		os.Exit(int(types.Unknown))
	}
	...
	if err != nil {
		fmt.Printf("error checking %v networkInterface: %v\n", hco.NetworkInterface, err)
		os.Exit(int(types.Unknown))
	}
	if !healthy {
		fmt.Printf("%v was down\n", hco.NetworkInterface)
