...
if [ "$install_golangci_lint" = true ]; then
    ...
    # 下载并解压
    temp_dir=$(mktemp -d)
    curl -sSfL "https://gh.yylt.gq/golangci/golangci-lint/releases/download/v$TARGET_VERSION/golangci-lint-$TARGET_VERSION-linux-amd64.tar.gz" | tar -xzf - -C "$temp_dir"
